...
async function getToken() {
  ...
  let data;
  try { data = JSON.parse(body); } catch { throw new Error("Nowcast вернул некорректный token response"); }
...
function extractLayers(xml) {
  const layers = [];
  const re = /<Layer\b[^>]*>[\s\S]*?<Name[^>]*>([^<]+)<\/Name>[\s\S]*?<Title[^>]*>([^<]*)<\/Title>[\s\S]*?<\/Layer>/gi;
  let match;
  while ((match = re.exec(xml))) {
    const name = match[1].trim();
    const title = match[2].trim();
    if (name && !layers.some((item) => item.name === name)) layers.push({ name, title });
  }
  return layers;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "image") {
      const url = new URL(`${NOWCAST}/baltrad_wsgi`);
      const allowed = ["SERVICE", "VERSION", "REQUEST", "LAYERS", "STYLES", "SRS", "CRS", "BBOX", "WIDTH", "HEIGHT", "FORMAT", "TRANSPARENT", "TIME", "ELEVATION"];
